...
public class KafkaMessageListener {
	   ...
	   @KafkaListener(topics = "${message.topic.name1}", groupId = "${message.group.name}")
	...
	@KafkaListener(groupId = "${message.group.name}",topics = "${message.topic.name2}")
	public void listenTopic3(@Payload  String message,
	   ...
	   @KafkaListener(/*groupId = "${message.group.name:profegroup}", NO SE PUEDE PONER EN MISMO GRUPO SI ELEGIMOS PARTITION*/
